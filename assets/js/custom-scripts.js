  ...
  */
  ...
  $("#contactForm").validator().on("submit", function (event) {
    if (event.isDefaultPrevented()) {
      ...
      formError();
      submitMSG(false, "Preencha o formulario corretamente");
    } else {
  ...
  function submitForm() {
    ...
    $.ajax({
      type: "POST",
      url: "https://formspree.io/f/xeqbrkbo",
      data: {
        name: name,
        email: email,
        message: message
      ...
      success: function (data) {
        console.log(data); // Exibe a resposta do Formspree no console para depuração
        if (data.ok) {
          // Se o envio for bem-sucedido, você pode atualizar a página ou fazer outra ação aqui
          alert("Mensagem Enviada com Sucesso!");
        } else if (data.next) {
          // Se a propriedade "next" estiver definida, redirecione para a URL indicada
          window.location.href = data.next;
        } else {
          // Se não houver "error" ou "next" definido, exiba uma mensagem genérica de erro
